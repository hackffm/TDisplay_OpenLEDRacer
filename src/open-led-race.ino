...
void espnowSendString(const char* text) {
  #ifdef ENABLEESPNOWTX
    quickEspNow.send(ESPNOW_BROADCAST_ADDRESS, (uint8_t*)text, strlen(text));
  #endif
}
...
void loop() {
  ...
  static int oldRacePhase = -1;
  if(oldRacePhase != (int)race.phase) {
    oldRacePhase = (int)race.phase;
    Serial.printf("race.phase = %d\r\n", oldRacePhase);
  }
  ...
  switch(race.phase) {
      ...
      { 
        ...
        if( race.cfg.startline ) { // Standalone: Always true - Network mode: only racetrack where race starts
          ...
          if(param_option_is_active(&tck.cfg,AUTOSTART_MODE_OPTION) || race.demo_mode ) { // Autostart parameters ON 
            ...
            // ...or if long pressed
            if(pstart > 0) {
              if(startPressMs > 2000) goOn=true;
            } else {
              startPressMs = 0;
            }
          };
    ...
    case RACING:
       {
        strip_clear( &tck, true ); // does draw ramp and box as well, but not cars
...
//#endif
  ...
  switch(DispAction++) {
    ...
    case 2:
    //  Disp.dmxBuf[1] = ((track.getPixelColor(0) >> 16) & 0xff);
    //  Disp.dmxBuf[2] = ((track.getPixelColor(0) >> 8) & 0xff);
    //  Disp.dmxBuf[3] = ((track.getPixelColor(0) >> 0) & 0xff);
    //  Disp.dmxBuf[4] = 0;
 ...
 // Serial.printf(" DT us:%d, DA:%d\r\n", micros() - start_us, DispAction);

  // cars: 
  //   speed: typ: 0.0 - 2.0 ... 3.0
  //   dist: pixel auf Maintrack, wird weitergezählt bei weiteren Runden
  //   dist_aux: pixel auf Auxtrack, wird resetet, läuft typ 0-60
  //   nlap:  startet bei 1.0 - 5.0 (aktuelle Runde)
...
void set_controllers_mode(uint8_t numctrl, uint8_t mode ) {
  for( uint8_t i = 0; i < numctrl; ++i) {
    controller_init( &switchs[i], (enum ctr_type)mode, DIGITAL_CTRL[i] );
  }
}
...
void run_racecycle( car_t *car, int caridx ) {
    ...
    if( car->st == CAR_RACING ) {
        ...
        car_updateController( car ); // Add speed to car
        draw_car( &tck, car );
...
/* 
 *  non-blocking, returns true if done  
 */
  ...
  if(countdown_new_phase){
    ...
    switch(countdown_phase) {
      ...
      case 4:
        ...
        tone(PIN_AUDIO,880); espnowSendString("DongDong:p");       
...
void sendResponse( ack_t *ack) {
  if(ack->type=='\0'){
    sprintf(txbuff, "%s%c", ack->rp==RSPOK? "OK":"NOK" , EOL );
  } else {
    sprintf(txbuff, "%c%s%c", ack->type, ack->rp==RSPOK? "OK":"NOK" , EOL );
  }
  serialCommand.sendCommand(txbuff);
}
